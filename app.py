...
# Set up the page
st.title('Safe Guarding Cyberspace')
st.write(
    'This ML-based app is Developed By Lingeswar KB for educational purposes as a Final year Project. '
    'Objective of the app is detecting phishing websites only using URL and its content data! '
...
with st.expander("PROJECT DETAILS"):
    ...
    st.write('Data set was created in April 2024.')
...
with st.expander('EXAMPLE PHISHING URLs:'):
    st.write('_https://secure-metmaskextnsn.webflow.io/_')
    st.write('http://xp.yfe13.za.com')
...
if st.button('Check!'):
    try:
        ...
        if response.status_code == 404:
            st.error("Invalid or expired URL!")
            ...
            if result[0] == 0:
                st.success("This web page appears to be legitimate and safe to use.")
                st.balloons()
            else:
                st.warning("Warning: This web page may be a potential phishing site. Please be cautious and double-check the URL before entering any sensitive information.")
                st.snow()
